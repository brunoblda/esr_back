...
def result_to_csv(list_issues, list_results):
    ...
    if (len(list_issues) == len(list_results)):
        ...
        if len(list_issues) == 1:
            ...
            csv_name = f"sla_results{random_number}.csv" 

        with open(csv_name, 'w', newline='') as csvfile:
            fieldnames = ['Sistema', 'Tarefa', 'Prioridade', 'Data de atribuicao', 'Tempo Liquido', 'Data de entrega para Homologacao',
             'Data de alterado para Resolvido', 'Delta_tempo','IPS(horas uteis de atraso)', 'Nivel de serviço dentro do acordo?', 'Feriado']
